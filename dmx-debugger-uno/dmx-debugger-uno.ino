/*
  DMX512 Live Monitor with TFT Display – Arduino Uno (ATMega328)
  ===============================================================
  Hardware:
    DMX input    : Serial0 / RX0 (Pin 0) via RS485 transceiver (RE/DE tied to GND)
                   Note: USB-Serial not available while DMX is connected.
  ...
  ── Required libraries – install via Library Manager ─────────────────────────
    ...
    1. DMXSerial             Matthias Hertel
       ...
       Uses Serial0 (hardware UART 0, Pin 0) — default, no change to DMXSerial_avr.h needed.
  ...
  __ Build and Upload
    ...
    --board arduino:avr:uno --port COM8 \
    --upload "C:\Users\rp-remote\Documents\Arduino\dmx-debugger-uno\dmx-debugger-uno.ino"
    ...
    Note: Pin 0 (RX0) is used for DMX — disconnect RS485 before uploading sketches.
...
static void drawP1Cell(uint16_t idx, uint8_t val) {
  ...
  const uint16_t barCol    = valueColor(val);
  const uint16_t bgCol     = C_BG;
  const uint16_t borderCol = C_GRID;
...
static void drawP23Cell(uint16_t pageChIdx, uint8_t val) {
  ...
  const uint16_t fg = valueColor(val);
  const uint16_t bg = C_BG;
...
static void drawAllP23Cells(uint8_t page) {
  ...
  for (uint16_t i = 0; i < count; i++) {
    drawP23Cell(i, snapshot[start + i]);
  }
}
...
void setup() {
  ...
  // Start DMX receiver on Serial0 (UART0, Pin 0)
